...
void Music_Play(const uint16_t, uint16_t , const struct Songs* song, uint16_t);
extern uint32_t play, rewind, mode, pause;
extern uint16_t i,j, lenPush;
  ...
  PolledButtons_Init();       // initialize interrupts and ports
	...
	while(1){
		if(mode){
			...
			if (instrument == 0){
				Music_Play(Wave[0], 64, strobePtr, 39);	
				instrument += 1;
			}
			else if (instrument == 1){
				Music_Play(Horn[0], 32, strobePtr, 39);
				instrument += 1;
			}
			else if (instrument == 2){
				Music_Play(Flute[0], 32, strobePtr, 39);
				instrument = 0;
			}
		}
		else if(play){
