...
// Data/Command (pin 4) connected to PA0 (GPIO)
// RESET (pin 3) connected to PA1 (GPIO)
...
#define DC                      (*((volatile uint32_t *)0x40004004))
#define DC_COMMAND              0
#define DC_DATA                 0x01
#define RESET                   (*((volatile uint32_t *)0x40004008))
#define RESET_LOW               0
...
void static commonInit(const uint8_t *cmdList) {
  ...
  delay = SYSCTL_RCGC1_R; // allow time for clock to start
	delay = SYSCTL_RCGC2_R;
	
  // toggle RST low to reset; CS low so it'll listen to us
  // SSI0Fss is temporarily used as GPIO
  GPIO_PORTA_DIR_R |= 0x0B;             // make PA0,1,3 out
  GPIO_PORTA_AFSEL_R &= ~0x0B;          // disable alt funct on PA0,1,3
  GPIO_PORTA_DEN_R |= 0x0B;             // enable digital I/O on PA0,1,3
                                        // configure PA0,1,3 as GPIO
  TFT_CS = TFT_CS_LOW;
  RESET = RESET_HIGH;
  Delay1ms(500);
  RESET = RESET_LOW;
